...
├── voices/                  # Voice profiles - PRIVATE, not committed (in .gitignore)
├── work/                    # Working files, experiments - PRIVATE, not committed (in .gitignore)
...
Two directories are excluded from git for personal/working content:

- **`voices/`** - Your voice profiles (.wav, .txt). Never committed to protect privacy.
- **`work/`** - Personal experiments, drafts, scripts with private content. Use for anything you don't want public.
